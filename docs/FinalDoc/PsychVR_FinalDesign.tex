...
\end{figure}
In our implementation, we optimized the generation process, by instantiating 100 chunks off camera at one position in the map 
Terrain movement is handled in all cardinal directions, when the user moves from the center tile the grid is redrawn so that the current tile is in the center, corresponding planes are removed and created, and the necessary planes are restitched. 
...
Each Chunk has a set of objects that are scaled and spawned randomly across the plane, the object include two trees, three rocks and mountains. The example script below shows the spawning process for a chunk, which is executed once its loaded. 
\begin{figure}[H]
...
\begin{itemize} %JOHN but nah he will never so ERIC
