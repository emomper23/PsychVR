...
\begin{itemize}
 ...
 \item As the user moves through the scene create and destroy chunks so the user can move in an infinite environemnt.
...
\begin{figure}[H]
	...
	\lstinputlisting [language = C] {chunkGen.txt}
	\caption{Terrain Chunk Gerneration Pseudo Code by Eric Momper}
\end{figure}
Due to our older version of Unity we are on 5.3.2f1 only supports meshes with upto 65,000 verticies so our terrain was size 200,200 with cell size of 2 (giving it a total of 60K vertices per mesh). A 3X3 grid of meshes are spawned and stitched together (explained in the next section). 


\pagebreak

\paragraph{Terrain stitching}
Terrain stitching is done to connect the verticies 
\begin{figure}[H]
	\label{fig:terrainStitchCode}
	\lstinputlisting [language = C] {stitch.txt}
	\caption{Chunk edge stitching pseudocode by Eric Momper}
\end{figure}
\begin{figure}[H]
	\label{fig:terrainMoveCode}
	\lstinputlisting [language = C] {terrainMove.txt}
