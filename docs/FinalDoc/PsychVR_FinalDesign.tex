...
\begin{itemize}
 ...
 \item As the user moves through the scene create, respawn, and restitch and destroy chunks so the user can move in an infinite environemnt.
\end{itemize}
...
\begin{figure}[H]
	...
	\lstinputlisting [language = C, basicstyle=\tiny] {chunkGen.txt}
	\caption{Terrain Chunk Gerneration Pseudo Code}
\end{figure}
Due to our hardware we have for the project (the Oculus Rift DK1) we are using an older version of Unity (5.3.2f1) which only supports meshes with upto 65,000 verticies so our terrain was size 200 X 200 with cell size of 2 (giving it a total of 60K vertices per mesh). A 3X3 grid of meshes are spawned and stitched together (explained in the next section). 

\pagebreak

\paragraph{Terrain stitching}
Terrain stitching is done to connect the verticies of adjacent chunks so they are connected at the edges, the terrain stitch code stitches the initial 3x3 grid and runs to restitch the newly created chunks as the user moves (example shown below)
\begin{figure}[H]
	\label{fig:terrainStitchCode}
	\centerline{\lstinputlisting [language = C, basicstyle=\tiny] {stitch.txt} }
...
\begin{figure}[H]
	\centerline{\lstinputlisting [language = C, basicstyle=\tiny] {terrainMove.txt}}
