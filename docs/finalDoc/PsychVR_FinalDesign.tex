...
\par ~\\
As our project will be used for therapy, one of our main objectives will be to give the user the most immersive, believable experience possible. We plan to use a few different systems to provide detailed modeling and tracking. We will use the Kinect for motion and skeleton tracking to allow the user to move around, the Gear VR for detailed hand and arm tracking, and the VR headset (Oculus or Samsung Gear) for rotation and head tracking. 
\par ~\\
...
%B. Technical objectives, goals, specifications, and requirements (mostly text and numbers) FIRST PAPER HERE EZPZ
	...
	\begin{itemize}
		...
		\item Additionally, our project will contain customization tools to allow users to build more of these from the ground up.
	\end{itemize}
	...
	\par~\\  For more information on the history of VR in psychology and how it relates to our project, see the Psychology section under Research.
	\pagebreak
...
\section{Project Goals and Objectives}
	...
	\begin{itemize}
		...
		\item Provide some sort of user stress analysis or user progress of treatment to the user, with analysis and resluts desplayed in the Qt app.
		...
		\item Implement Module 2 Speech Therapy which will simulate public speeking or various verbal social situations. 
	...
	Our project would have an impact on the field of psychology as it relates to certain emotional disorders or struggles and the deployment of self-administered 
	therapy for those disorders. It would not only be relevant to people diagnosed with a psychological condition, but also those seeking stress relief or a unique 
	environment to immerse themselves in. By bringing treatment therapies to the end user in their own home and allowing them to perform therapy at their leisure we will 
	have an impact on many people seeking these services who may not have the time to schedule appointments or those who simply won't try due to the stigma surrounding therapy.
	\pagebreak
	...
	\begin{enumerate}
	 ...
	 \item Accessibility of Guides
	  \begin{itemize}
	    ...
	    \item  Dependency:Guide Quality and accessiblity.  
...
\begin{figure}[H]
	...
	\label{fig:kimg}
	\end{figure}
	For our project we will use the Kincet for user body and movement tracking. The kinect version 1 and 2 (for the Xbox 360/ Xbox one) are both powerful devices that have many 
...
A Kinect streams out color, depth, and skeleton data one frame at a time. 
\begin{figure}[H]
...
\item The depth sensor has two depth ranges: the default range and the near range The default range is available in both the Kinect 1 and 2 the near range is available only in the Kinect 2.
...
\item Floor Determination

Each skeleton frame also contains a floor-clipping-plane vector, which contains the coefficients of an estimated floor-plane equation. It is used as a clipping plane for
removing the background and segmenting players.
The general plane equation is:
\begin{equation}
  Ax + By + Cz + D = 0 
\end{equation}
\centerline{ Where A,B,C,D are the coordiantes on the floor clip points.}

The equation is normalized so that the physical interpretation of D is the height of the camera from the floor, if the floor is not detectable 
...
\subsection{Budget and Financing}
	...
	prototying for the different modules and testing different VR platforms, sensors, and controls as we can acquire them. Some basic builing of modules and design has been done
	this semester, but no prototypes developed so far have encompassed multiple requirements, they just acted as a proof of concept for the differnt technologies or ideas.
	\pagebreak
	\section{Milestones}
