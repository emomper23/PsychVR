...
public class AvatarController : MonoBehaviour
{
    ...
    public Transform front_bound;
    public Transform back_bound;
    ...
    void MoveAvatar(uint UserID)
    {
        ...
        Debug.Log(Root.position.z + " " + front_bound.position.z + " " + back_bound.position.z);
        ...
        if (Root.position.z < front_bound.position.z && Root.position.z > back_bound.position.z)
        {
            zPos = -trans.z * MoveRate - ZOffset;
        }
        else
        {
            zPos = Root.localPosition.z*0.95f;
        }
